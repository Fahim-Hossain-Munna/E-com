...
onMounted(async()=>{
    ...
    if(urlPath){
        ...
        let categoryProductResponse = await axios.get("https://dummyjson.com/products?limit=12")
        products.value = categoryProductResponse.data.products
    }
   ...
   <div class="my-[140px]">
        <Container>
            ...
            <div class="lg:flex block lg:flex-wrap justify-between items-center gap-y-20 px-2 lg:px-0">
                <Product v-for="product in products" :key="product.id" :imgSrc=product.thumbnail :title=product.title :price=product.price :discount=product.discountPercentage />
            </div>
        </Container>
   </div>
</template>